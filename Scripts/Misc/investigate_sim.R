...
out_dir <- getwd() %+% "/Simulation/Error_Check_Sim/Output_no_repartner_w_corr/"
...
for(i in 1:200){
  ...
  js_sim_list[[i]]    <- readRDS(out_dir %+% js_file)
...
library(ggmcmc)
ps_sim_list[[11]]$samples %>% ggs() %>% filter(Parameter %in% c("beta0","beta1")) %>% ggs_traceplot() + ylim(-5,5)
ps_sim_list[[11]]$samples %>% ggs() %>% filter(Parameter %in% c("PhiF","PhiM","PF","PM")) %>% ggs_traceplot()#+ ylim(0.65,0.99)
ps_sim_list[[11]]$samples %>% ggs() %>% filter(Parameter %in% c("delta")) %>% ggs_traceplot() + ylim(0,1)
ps_sim_list[[11]]$samples %>% ggs() %>% filter(Parameter %in% c("eps[" %+% 1:jags_data$k %+% "]")) %>% ggs_traceplot() + ylim(0,1)
ps_sim_list[[11]]$samples %>% ggs() %>% filter(Parameter %in% c("gamma","rho")) %>% ggs_traceplot() #+ ylim(-1,1)
ps_sim_list[[11]]$samples %>% ggs() %>% filter(Parameter %in% c("gamma_raw","rho_raw")) %>% ggs_traceplot() + ylim(0,1)
...
p1 <- ps_results %>% filter(Parameter == "PM", scenario == 3) %>%
  ggplot() +
...
p2 <- ps_results %>% filter(Parameter == "rho", scenario == 1) %>%
  ggplot() +
  geom_line(aes(x = iteration, y = `2.5%`),linetype = "dashed") + 
  geom_line(aes(x = iteration, y = `97.5%`),linetype = "dashed") +
  geom_line(aes(x = iteration, y = Mean)) +
