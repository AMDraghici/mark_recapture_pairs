...
nimble_dat <- list(
  ...
  apairs = jags_data$apairs,
  af = jags_data$af,
  am = jags_data$am,
  afmat = jags_data$afmat,
  ammat = jags_data$ammat,
  apairs = jags_data$apairs,
  recap_f = jags_data$recap_f,
  recap_m = jags_data$recap_m,
  rfmat = jags_data$rfmat,
  rmmat = jags_data$rmmat
)

nimble_params <- c("phiF", "phiM", "pF", "pM", "gamma","rho")
...
samples <- runMCMC(CcjsMCMC, niter = 100, nburnin = 50, thin = 1, setSeed = TRUE,samplesAsCodaMCMC = TRUE)
