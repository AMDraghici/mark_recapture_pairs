...
simulate_cr_data <- function(n,
  ...
  # Partner Perish interval (if we see a swap a death must have occurred)
  perish_interval <- matrix(0, nrow = nf, ncol = k)
  for(i in 1:nf){
    for(t in first_capture_f[i]:(k-1)){

      mask1 <- (last_partner_matrix[i,t] != next_partner_matrix[i,t])
      mask2 <- (last_partner_matrix[i,t] != (nm + 1))
      mask3 <- (next_partner_matrix[i,t] != (nm+1))
      mask4 <- (length(which(!is.na(apairs_f[i,(t+1):k]))) != 0)

      # If partners dont match, not a single interval, and not at the end of study
      if(mask1 & mask2 & mask4){
        perish_interval[i,t] <- 1
      }
    }
  }

  # Define bounds of a new interval
  perish_interval_start <- matrix(0, nrow = nf, ncol = k)
  perish_interval_end <- matrix(0, nrow = nf, ncol = k)

  for(i in 1:nf){
    for(t in (first_capture_f[i]):(k-1)){
      if(!is.na(apairs_f[i,t]) & is.na(apairs_f[i,t+1])){
        if((length(which(!is.na(apairs_f[i,(t+1):k]))) != 0)){
          if(perish_interval[i,t]==1){
            perish_interval_start[i,t] <- t
          }
        }
      }
      ...
      if(is.na(apairs_f[i,t]) & !is.na(apairs_f[i,t+1])){
        if(perish_interval[i,t]==1){
          perish_interval_end[i,t] <- t
        }
      }
    }
    
    # Backfill perish_interval_ending
    for(t in first_capture_f[i]:(k-1)){
      if(perish_interval_start[i,t] != 0 & perish_interval[i,t]==1){
        
        perish_ending_spots <- which(perish_interval_end[i,1:k] != 0 & perish_interval[i,1:k] == 1)
        perish_ending_spots <- perish_ending_spots[perish_ending_spots > t]
        if(length(perish_ending_spots) != 0){
          perish_interval_start[i,(t+1):perish_ending_spots[1]] <- perish_interval_start[i,t]
          perish_interval_end[i,t:(perish_ending_spots[1]-1)] <- perish_interval_end[i, perish_ending_spots[1]]
        }
      }
    }
  }
  ...
  for(i in 1:nf){
    ...
    known_pair_index_i <- which(!is.na(apairs_f[i,first_capture_f[i]:k]))
    
    # If no partners ever observed just assume unknown for entire study
    if(length(known_pair_index_i) == 0){
      next
    }
    ...
    for(t in first_capture_f[i]:(k-1)){
      # If pair known then....
      if(!is.na(apairs_f[i,t])){
        # If single set to single status...
        if(apairs_f[i,t] == (nm+1)){
          az[i,t] <- 1
          next
        } else {
          # If partner at time t+1 is known...
          if(!is.na(apairs_f[i,t+1])){
            # If next partner is different male then this male must have died here..
            if(apairs_f[i,t+1] != apairs_f[i,t]){
              az[i,t] <- 2 
              
              # The truth should be that the male died, if not check for bug
              if(any(sm[apairs_f[i,t],(t+1):k] != 0)) browser()
              if(any(recap_m[apairs_f[i,t],(t+1):k] != 0)) browser()
              
              # Update our hidden data vector to reflect this
              am[apairs_f[i,t],(t+1):k] <- 0
              
              next
            } else {
            # If next partner is same male then this male must have survived here..
              az[i,t] <- 3
              if(sm[apairs_f[i,t],t+1] != 0)
              am[apairs_f[i,t],t+1] <- 1
              next
            }
            
          # If partner at time t+1 is unknown
          } else {
            # When is she seen with another partner
            index_of_future_pairs <- known_pair_index_i[known_pair_index_i >= (t+1)]
            
            # If never then leave rest as unknown
            if(length(index_of_future_pairs) != 0){
              
              if(apairs_f[i,index_of_future_pairs[1]] != apairs_f[i,t]){
                if(any( sm[apairs_f[i,t], index_of_future_pairs[1]:k] != 0)) browser()
                if(any( recap_m[apairs_f[i,t], index_of_future_pairs[1]:k] != 0)) browser()
                am[apairs_f[i,t], index_of_future_pairs[1]:k] <- 0
              } else {
                az[i,t] <- 3
                if(any(sm[apairs_f[i,t], (t+1):index_of_future_pairs[1]] == 0)) browser()
                am[apairs_f[i,t], (t+1):index_of_future_pairs[1]] <- 1
              }
            } 
          }
        }
  ...
  model_data <- list(
    ...
    perish_interval_start = perish_interval_start,
    perish_interval_end   = perish_interval_end,
