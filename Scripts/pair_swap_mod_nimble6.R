...
nimble_ps_model <- nimbleCode({
  ...
  for(i in 1:nf){
    ...
    # Sample Female Mate 
    psi_cond[i, 1:(nm+1), first_capture_f[i]] <- compute_psi_cond_it(psi[i,1:(nm+1),first_capture_f[i]],
    ...
    apairs_f[i,first_capture_f[i]] ~ dcat(psi_cond[i, 1:(nm+1), first_capture_f[i]])
    # apairs_f[i,first_capture_f[i]] <- which((psi_cond[i, 1:(nm+1), first_capture_f[i]]) == 1)[1]
    # apairs_f[i,first_capture_f[i]] <- assign_partner(psi_cond[i, 1:(nm+1), first_capture_f[i]], nm)
    ...
    for(t in (first_capture_f[i]+1):k){
      ...
      # apairs_f[i,t] <- assign_partner(psi_cond[i, 1:(nm+1), t], nm)
  ...
  for(t in 1:k){
    ...
    for(i in 1:nf){
      ...
      if(t==first_capture_f[i]){
        ...
        # By construction psi must pick ONE partner after conditioning
        # if(sum(psi_cond[i, 1:(nm+1), first_capture_f[i]]) != 1) browser()
        
        apairs_f[i,first_capture_f[i]] <- ifelse(is.na(apairs_f[i,first_capture_f[i]]),
                                                 rcat(n=1, psi_cond[i, 1:(nm+1), first_capture_f[i]]), #assign_partner(psi_cond[i, 1:(nm+1), first_capture_f[i]], nm), #rcat(n =1 , psi_cond[i, 1:(nm+1), first_capture_f[i]]),
        ...
        # if(sum(psi_cond[i, 1:(nm+1), t]) != 1) browser()
        apairs_f[i,t] <- ifelse(is.na(apairs_f[i,t]),
                                rcat(n=1, psi_cond[i, 1:(nm+1), t]), #assign_partner(psi_cond[i, 1:(nm+1), t], nm), # ,
                                apairs_f[i,t])
...
run_pair_swap_nimble_parallel <- function(data, params, niter, nthin, nburnin, ncores){
  ...
  clusterEvalQ(cl, {
    ...
    source(paste0(getwd(), "/Scripts/pair_swap_mod_nimble6.R"))
