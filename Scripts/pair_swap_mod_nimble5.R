...
compute_prob_condF <- nimbleFunction(
  run = function(is_single_female = double(1),
    ...
    out <- is_single_female[1:nf] * ProbF + 
           (Ind1-is_single_female[1:nf]) * (current_male_state[current_pairs_f] * (Probfm/ProbM) + 
                                           (Ind1- current_male_state[current_pairs_f]) * (Probf0/(1-ProbM)))
...
compute_psi_cond_it <- nimbleFunction(
  run = function(psi_it = double(1),
    ...
    for(j in 1:nm){
      j_taken <- sum(vectorMatch(current_pairs, j))
      psi_cond_it[j] <- psi_it[j] * amating_m[j] * amating_f * (1-j_taken)
    }
    
    # If amating_f = 1 and pairs are possible drop this case
    if(sum(psi_cond_it[1:nm]) > 0){
      psi_cond_it[nm+1] <- psi_it[nm+1] * (1-amating_f)
    }
...
nimble_ps_model <- nimbleCode({
  ...
  for(i in 1:nf){
    for(t in 1:k){
      amating_f[i,t] ~ dbern(delta * recruit_f[i,t] * af[i,t])
    }
  }
  ...
  for(j in 1:nm){
    for(t in 1:k){
      amating_m[j,t] ~ dbern(delta * recruit_m[j,t] * am[j,t])
    }
  }
  ...
  for(t in 1:k){
    ...
    # Sample Female Mate 
    psi_cond[1, 1:(nm+1), t] <- compute_psi_cond_it(psi[1,1:(nm+1),t],
                                                    amating_f[1,t],
                                                    amating_m[1:nm,t],
                                                    c(0,0),
                                                    nf,
    ...
    apairs_f[1,t] ~ dcat(psi_cond[1, 1:(nm+1), t])
    ...
    for(i in 2:nf){
      ...
      # Partner also needs to be available
      psi_cond[i,1:(nm+1),t] <- compute_psi_cond_it(psi[i,1:(nm+1),t],
                                                    amating_f[i,t],
                                                    amating_m[1:nm,t],
                                                    apairs_f[1:(i-1),t],
                                                    nf,
      ...
      apairs_f[i,t] ~ dcat(psi_cond[i,1:(nm+1),t])
    }
...
generate_nimble_init_pairs <- function(ps_data){
  ...
  for(t in 1:k){
    ...
    if(is.na(apairs_f[1,t])){
      apairs_f[1,t] <- rcat(n = 1, psi_cond[1, 1:(nm+1), t])
    }
    ...
    for(i in 1:nf){
      ...
      if(is.na(apairs_f[i,t])){
        apairs_f[i,t] <- rcat(n = 1, psi_cond[i, 1:(nm+1), t])
      }
    }
  ...
  # Partnership
  apairs_f <- build_NA_mat(apairs_f, ps_data$apf)
...
run_pair_swap_nimble_parallel <- function(data, params, niter, nthin, nburnin, ncores){
  ...
  clusterEvalQ(cl, {
    ...
    source(paste0(getwd(), "/Scripts/pair_swap_mod_nimble5.R"))
